...
def calib_sensitivity(model, tokenizer,args):
    ...
    cache_file = f"cache/{model_id.replace('/','_')}_sensitivity_s{args.test_split}_alpha.pt"
    if os.path.exists(cache_file) and not args.disable_cache:
        sensitivity_dict,alpha_dict = torch.load(cache_file, map_location="cpu")
        return sensitivity_dict,alpha_dict
    model.eval()
    ...
    alpha_candidates = [0, 0.1, 0.5, 1, 2, 3]
    sensitivity_dict = {}
    alpha_dict={}
    ...
    for raw_linear, info in linear_info.items():
        ...
        for alpha in alpha_candidates:
            ...
            if best_alpha_ppl is None or ppl<best_alpha_ppl:
                best_alpha=alpha
                best_alpha_ppl=ppl
            print(f"{info['full_name']} {alpha} {ppl}")
        print(f"best_alpha={best_alpha}, best_alpha_ppl={best_alpha_ppl}")
        alpha_dict[info["full_name"]]=best_alpha
    ...
    torch.save((sensitivity_dict,alpha_dict), cache_file)
    return sensitivity_dict,alpha_dict


def search_best_compression_ratio(model, tokenizer, sensitivity_dict,alpha_dict, args):
    path = f"output/{args.model_id.replace('/','_')}"
    if not os.path.exists(path):
        os.makedirs(path)
    log_file = open(
        f"{path}/ssearch_a{args.act_aware}_s{args.test_split}_alpha.json",
        "a+",
    )
...
def main(args):
    ...
    sensitivity,alpha_dict = calib_sensitivity(model, tokenizer,args)
    ...
    search_best_compression_ratio(model, tokenizer, sensitivity,alpha_dict, args)
